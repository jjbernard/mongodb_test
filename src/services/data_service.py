...
def register_cage(active_account: Owner, name: str, allow_dangerous,
    ...
    cage.allow_dangerous_snake = allow_dangerous
    cage.price = price
    cage.save()
...
def get_available_cages(checkin: datetime.datetime,
    ...
    if snake.is_venomous:
        query = query.filter(allow_dangerous_snake=True)
...
def book_cage(account: Owner, snake, cage, checkin, checkout):
...
def get_bookings_for_user(email: str) -> List[Booking]:
    ...
    def _map_cage_to_booking(cage, booking):
        booking.cage = cage
        return booking

    bookings = [
        _map_cage_to_booking(cage, booking)
